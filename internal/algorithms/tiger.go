...
func TigerHash(message []byte) [24]byte {
	...
	// ho, h1 i h2 su inicijalni a, b i c respektivno.
	//Inicijalizacija promenljivih:
	h0 := uint64(0x0123456789ABCDEF)
	h1 := uint64(0xFEDCBA9876543210)
	h2 := uint64(0xF096A5B4C3D2E187)

	// Preprocesiranje. Svodjenje poruke na dužinu koja je umnožak od 512
	// (u bitovima). Može se uraditi bilo kako, ali dva najčešća načina su
	// dodavanje potrebnog broja nula na kraju ili sledeće:
	paddedMessage := padMessage(message)

	//Procesirati poruku (message) kao niz 512-bitnih segmenata:
	for i := 0; i < len(paddedMessage); i += 64 {
		chunk := paddedMessage[i : i+64]
		...
		for i := range 4 {
			...
			for j := range 8 {
				...
				a = a - (table[c0] ^ table[256+c2] ^ table[512+c4] ^ table[768+c6])
				b = b + (table[768+c1] ^ table[512+c3] ^ table[256+c5] ^ table[768+c7])
				b = b * uint64(i+1)
			}
	...
	var digest [24]byte
	binary.LittleEndian.PutUint64(digest[0:8], h0)
	binary.LittleEndian.PutUint64(digest[8:16], h1)
	binary.LittleEndian.PutUint64(digest[16:24], h2)
...
func padMessage(message []byte) []byte {
	...
	padded = append(padded, 0x80) //TODO: IDK? tehnicki je 10000000, a ne 1

	// -append "0" bits until message length in bits ≡ 448 (mod 512)
	// ili posto radimo u bajtovima ovde: bits ≡ 56 (mod 64)
